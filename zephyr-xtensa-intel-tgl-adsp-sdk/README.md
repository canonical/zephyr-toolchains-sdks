# Zephyr xtensa-intel-tgl-adsp Toolchain SDK for Workshop

This SDK provides the `xtensa-intel_tgl_adsp_zephyr-elf` cross-compiler toolchain for building Zephyr
...
2. Connect this SDK's `toolchain` slot to the matching toolchain plug so the
   `xtensa-intel_tgl_adsp_zephyr-elf` cross-compiler becomes available inside the workshop.
...
- Interface: `mount`
- Workshop source: `$SDK`
- Purpose: Provides the `xtensa-intel_tgl_adsp_zephyr-elf` cross-compiler to the `zephyr` base SDK.
