# Zephyr xtensa-mtk-mt8195-adsp Toolchain SDK for Workshop

This SDK provides the `xtensa-mtk_mt8195_adsp_zephyr-elf` cross-compiler toolchain for building Zephyr
...
2. Connect this SDK's `toolchain` slot to the matching toolchain plug so the
   `xtensa-mtk_mt8195_adsp_zephyr-elf` cross-compiler becomes available inside the workshop.
...
- Interface: `mount`
- Workshop source: `$SDK`
- Purpose: Provides the `xtensa-mtk_mt8195_adsp_zephyr-elf` cross-compiler to the `zephyr` base SDK.
