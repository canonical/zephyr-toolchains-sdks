...
if [ -n "$MODULE_YML" ]; then
  # Use a module.yml provided locally (e.g. extracted from the hal-espressif
  # PPA deb) instead of downloading it from GitHub.
  if [ ! -f "$MODULE_YML" ]; then
    echo "Error: MODULE_YML=$MODULE_YML does not exist"
    exit 1
  fi
  YAML_FILE="$MODULE_YML"
  echo -e "Using local module.yml: $YAML_FILE\n"
else
  # Fall back to fetching module.yml from GitHub at a pinned commit.
  if [ -z "${HAL_ESPRESSIF_COMMIT:-}" ]; then
    echo "Error: set MODULE_YML (local file) or HAL_ESPRESSIF_COMMIT (GitHub fetch)"
    exit 1
  fi
