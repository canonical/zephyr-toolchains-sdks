# Zephyr xtensa-amd-acp-6-0-adsp Toolchain SDK for Workshop

This SDK provides the `xtensa-amd_acp_6_0_adsp_zephyr-elf` cross-compiler toolchain for building Zephyr
...
2. Connect this SDK's `toolchain` slot to the matching toolchain plug so the
   `xtensa-amd_acp_6_0_adsp_zephyr-elf` cross-compiler becomes available inside the workshop.
...
- Interface: `mount`
- Workshop source: `$SDK`
- Purpose: Provides the `xtensa-amd_acp_6_0_adsp_zephyr-elf` cross-compiler to the `zephyr` base SDK.
